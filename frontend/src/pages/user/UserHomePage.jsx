...
const UserHomePage = () => {
  ...
  const initialTab = searchParams.get("tab") || location.state?.activeTab || "rooms";
  ...
  const userMenuItems = [
        ...
        <div style={{ padding: "6px 8px 10px 8px" }}>
          ...
          {activeTenancies.length > 0 && (
            <Tag color="success" style={{ marginTop: 6, borderRadius: 4, fontWeight: 600, fontSize: 11 }}>
              Đang thuê {activeTenancies.length} phòng
            </Tag>
          )}
        </div>
      ),
    ...
    { type: "divider" },
    {
      ...
      label: <span style={{ fontWeight: activeTab === "rooms" ? 700 : 500 }}>Phòng của tôi</span>,
      onClick: () => handleTabChange("rooms"),
    },
    {
      key: "contracts",
      icon: <FileProtectOutlined style={{ color: "#2563eb", fontSize: 16 }} />,
      label: <span style={{ fontWeight: activeTab === "contracts" ? 700 : 500 }}>Hợp đồng ({contracts.length})</span>,
    ...
    },
    {
      ...
      label: (
        <Space size={8}>
          <span style={{ fontWeight: activeTab === "invoices" ? 700 : 500 }}>Hóa đơn</span>
    ...
    },
    {
      ...
      label: (
        <Space size={8}>
          <span style={{ fontWeight: activeTab === "repair-requests" ? 700 : 500 }}>Báo sự cố</span>
      ...
      onClick: () => handleTabChange("repair-requests"),
    },
    {
      key: "room-requests",
      icon: <CreditCardOutlined style={{ color: "#0284c7", fontSize: 16 }} />,
      label: <span style={{ fontWeight: activeTab === "room-requests" ? 700 : 500 }}>Yêu cầu & Cọc ({roomRequests.length})</span>,
      onClick: () => handleTabChange("room-requests"),
    },
    {
      key: "interested-rooms",
      icon: <HeartOutlined style={{ color: "#e11d48", fontSize: 16 }} />,
      label: <span style={{ fontWeight: activeTab === "interested-rooms" ? 700 : 500 }}>Phòng yêu thích ({interestedRooms.length})</span>,
      onClick: () => handleTabChange("interested-rooms"),
    },
    {
      key: "profile",
      icon: <UserOutlined style={{ color: "#4f46e5", fontSize: 16 }} />,
      label: <span style={{ fontWeight: activeTab === "profile" ? 700 : 500 }}>Hồ sơ cá nhân</span>,
  ...
  return (
    <>
      ...
      <div className="quick-action-grid">
        ...
        <div className="quick-action-card" onClick={() => navigate("/user/repair-requests")}>
